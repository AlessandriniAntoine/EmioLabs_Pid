...
$$ f(k+1) = \frac{dt}{\tau + dt} f(k) + (1 - \frac{dt}{\tau + dt}) g(k) $$
where $f(k)$ is the filtered signal at time step $k$, $g(k)$ is the signal at time step $k$, and $dt$ is the simulation time step.
